...
class MultiRunner(object):
    ...
    def __init__(self, agents, environment, evaluation_environment=None):
        if not isinstance(agents, list):
            raise AttributeError('agents must be a list of agents')
        
        # if not isinstance(agents[0], Agent):
        ...
        for agent in self.agents:
            agent.initialize()
        self.global_episode = 0#self.agent.episode
        self.global_timestep = 0#self.agent.timestep
    ...
    def run(
        ...
        if evaluation_frequency is None:
            if update_as_evaluation_frequency:
                ...
                batch_size = self.agents[0].model.update_mode['batch_size']
                frequency = self.agents[0].model.update_mode.get('frequency', batch_size)
                if self.agents[0].model.update_unit == 'episodes':
        ...
        while True:
            ...
            if self.episode % self.evaluation_frequency == 0:
                ...
                self.global_timestep = self.agents[0].timestep
                self.global_episode = self.agents[0].episode
                ...
                if self.save_best_agent:
                    ...
                    if self.best_evaluation_score is None:
                        ...
                        self.best_evaluation_score = evaluation_score
                        self.agents[0].save(filename='best-model', append_timestep=False)
                else:
            ...
            if self.global_timestep >= self.num_timesteps:
                ...
                return
            elif self.agents[0].should_stop():
                return
    ...
    def run_episode(self, environment, max_timesteps, evaluation):
        ...
        # Get the index of to act agent
        to_act = int(states['to_act'])

        first_move_done = [False for a in self.agents]

        # Timestep loop
        while True:
            # Retrieve actions from agent
            actions = self.agents[to_act].act(
            ...
            if not evaluation and first_move_done[to_act]:
                self.agents[to_act].observe(terminal=False, reward=0)
            ...
            for _ in range(self.num_repeat_actions):
                ...
                reward += step_reward[to_act]
                if terminal:
                    for index, value in enumerate(step_reward):
                        self.agents[index].observe(terminal=True, reward=value)
                    break
            self.episode_reward += step_reward[0]

            first_move_done[to_act] = True
            ...
            # Update global timestep/episode
            self.global_timestep = self.agents[to_act].timestep
            self.global_episode = self.agents[to_act].episode
            ...
            if self.global_timestep >= self.num_timesteps:
                ...
                return
            elif self.agents[to_act].should_stop():
                return False
